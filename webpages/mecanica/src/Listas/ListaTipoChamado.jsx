...
export default function ListaTipoChamado() {
  ...
  const carregarTiposChamado = async () => {
    try {
      ...
      setTiposChamado(Array.isArray(response.data) ? response.data : response.data.tiposChamado || []);
      setError(null);
    } catch (error) {
            ...
            {tiposChamado.map((tipo) => (
              <TableRow key={tipo.id}>
                ...
                <TableCell>
                  <IconButton
                    color="primary"
                    onClick={() => navigate(`/editar-tipo-chamado/${tipo.id}`)}
                  >
                    <EditIcon />
                  </IconButton>
                  <IconButton
